...
class CombinedBaseNode(BaseModel):
	...
	node_type: int = Field(description='DOM node type')
	...
	def get_accessibility_role(self) -> str | None:
		...
		if self.accessibility and 'role' in self.accessibility:
			return str(self.accessibility['role'])
		return None

	def get_accessibility_name(self) -> str | None:
		"""Get the accessibility name if available."""
		if self.accessibility and 'name' in self.accessibility:
			return str(self.accessibility['name']) if self.accessibility['name'] else None
		return None

	def get_accessibility_description(self) -> str | None:
		"""Get the accessibility description if available."""
		if self.accessibility and 'description' in self.accessibility:
			return str(self.accessibility['description']) if self.accessibility['description'] else None
		return None
